...
func (r *redisEventRepository) GetEventsForUserAfter(ctx context.Context, userID, lastEventID string) ([]*domain.Event, error) {
	start := "-"
	if lastEventID != "" {
		start = fmt.Sprintf("(%s", lastEventID)
	}

	messages, err := r.client.XRange(ctx, userStreamKey(userID), start, "+").Result()
	if err != nil {
		return nil, fmt.Errorf("failed to XRange stream for user %s: %w", userID, err)
	}
	...
	for _, msg := range messages {
		...
		if err != nil {
			log.Printf("Error converting stream message to event: %v", err)
			continue
		}
...
func (r *redisEventRepository) GetEventsForUserBlocking(ctx context.Context, userID, lastEventID string, timeout time.Duration) ([]*domain.Event, error) {
	if lastEventID == "" {
		lastEventID = "0-0"
	}

	streams, err := r.client.XRead(ctx, &redis.XReadArgs{
		Streams: []string{userStreamKey(userID), lastEventID},
		Count:   100,
		Block:   timeout,
	}).Result()
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to XRead from stream for user %s: %w", userID, err)
	}
